...
while ($true) {
    ...
    # Pastikan folder logs terdeteksi
    if (!(Test-Path "$ScriptDir\logs")) { New-Item -ItemType Directory -Path "$ScriptDir\logs" -Force | Out-Null }
    ...
    :MenuLoop while ($true) {
        ...
        switch ($InputCmd.ToUpper()) {
            ...
            "E" { 
                ...
                $Samples = @()
                for ($i = 1; $i -le 10; $i++) {
                    Write-Progress -Activity "PERFORMANCE SCAN" -Status "Collecting real-time data... ($i/10 seconds)" -PercentComplete ($i * 10)
                ...
                Write-Progress -Activity "PERFORMANCE SCAN" -Completed
                ...
                try {
                    ...
                    if ($WinsatOut) {
                        $ReadMatch = ($WinsatOut | Select-String "Disk\s+Sequential\s+[0-9\.]+\s+Read\s+([0-9\.]+)\s+MB/s")
                        if ($ReadMatch) { $DeepStorageRead = "$([math]::Round($ReadMatch.Matches.Groups[1].Value, 2)) MB/s" }
                        
                        $WriteMatch = ($WinsatOut | Select-String "Disk\s+Sequential\s+[0-9\.]+\s+Write\s+([0-9\.]+)\s+MB/s")
                        if ($WriteMatch) { $DeepStorageWrite = "$([math]::Round($WriteMatch.Matches.Groups[1].Value, 2)) MB/s" }
                        
                        $LatMatch = ($WinsatOut | Select-String "Latency:\s+95th Percentile\s+([0-9\.]+)\s+ms")
                        if ($LatMatch) { $DeepStorageResponse = "$([math]::Round($LatMatch.Matches.Groups[1].Value, 2)) ms" }
                    }
                }
                catch {}

                $ServicesAuto = (Get-Service | Where-Object { $_.StartType -eq 'Automatic' -and $_.Status -eq 'Running' } | Sort-Object Name | ForEach-Object { "- $($_.Name) ($($_.DisplayName))" }) -join "`r`n"
                ...
                $DisksString = ""
                if ($Disks) {
                    $DisksString = ($Disks | ForEach-Object { "Drive $($_.DeviceID): $([math]::Round($_.FreeSpace / 1GB, 2)) GB free of $([math]::Round($_.Size / 1GB, 2)) GB ($([math]::Round(($_.FreeSpace / $_.Size) * 100, 0))%)" }) -join "`r`n"
                }

                $NetworkAdaptersString = ""
                if ($NetworkAdapters) {
                    $NetworkAdaptersString = ($NetworkAdapters | ForEach-Object { $_.Name }) -join "`r`n"
                }

                $PageFilesString = "Page File : None / Disabled"
                if ($PageFiles) {
...
Health / Wear Level: $BatteryWearLevel
